...
export default function () {
  group("Load test group", () => {
    ...
    res = http.get(`http://127.0.0.1:3001/post/${resourceId}`, {
    ...
    // Cập nhật resource
    res = http.patch(
      `http://127.0.0.1:3001/post/${resourceId}`,
      JSON.stringify({
    ...
    check(res, {
      "status is 200": (r) => r.status === 200,
      "resource updated": (r) => r.json("affected") === 1,
    });
    sleep(1);

    // Xóa resource
    // http://127.0.0.1:3001/post
    res = http.del(`http://127.0.0.1:3001/post/${resourceId}`, null, {
      headers: {
        "Content-Type": "application/json",
      },
      tags: { name: "UpdateResource" },
